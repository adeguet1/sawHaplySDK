# sawHaplySDK
SAW wrapper for Force Dimension haptic devices - the Haply SDK happens to support the devices manufactured by Haply (Sigma, Omega, ...) as well as the Novint Falcon.  This code compile on Linux and Windows.  This repositor provides a core component as well as:
...
this build will skip *sawHaplySDK* because the directory
...
```sh
adeguet1@lcsr-qla:~/catkin_ws$ ccmake build/saw_haply_sdk
```
In the command above, the ROS workspace is `~/catkin_ws` and the build tree is `build`.

Once in CMake, locate `haply_sdk_DIR` and make it point to
the directory containing your SDK.  For example,
`~/Haply/sdk-3.6.0`.  Hit `c`onfigure once and the two
variables `haply_sdk_LIBRARY_DHD` and
`haply_sdk_LIBRARY_DRD` should have been found
automatically.
...
that depends on the `sawHaplySDK` package.
...
sawHaplySDK will fail to build until you specify the path
`haply_sdk_DIR`.  After your first colcon run, use `ccmake
~/ros2_ws/build/sawHaplySDKCore`.  You will then be able to
...
The main example provided is `sawHaplySDKQtExample`.  The command line options are:
```sh
sawHaplySDKQtExample:
...
```sh
sawHaplySDKQtExample -j myconfig.json
```
...
A simple ROS client is provided in this package.  Once the ROS `haply` node is started, one can do (in Python):
```python
import crtk, haply
ral = crtk.ral('test')
f = haply.arm(ral, 'Falcon00') # use rostopic list to verify device's name
...
directions.  After you started the ROS node `haply
haply`, you can run the script/node `haply
xyz-motions.py` (e.g. for ROS 2 `ros2 run haply
